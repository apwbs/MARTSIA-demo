...
if [ -z "$slice_id" ]; then
  # Check if the last cipher has multiple slices
  ...
  if ! grep -q "| slice1" "../src/.cache"; then
    echo "The last encrypted message has no slices! Remove --slice-id!"
    exit 1
  elif  [ ${#slice_id} -lt 10 ] || ! echo "$slice_id" | grep -qE '^[0-9]+$'; then
